...
const escalationCategories: EscalationCategory[] = [
    { name: "Unfilled Critical Roles", percentage: 32, color: "var(--accent-red)", bgColor: "rgba(248, 81, 73, 0.1)", textColor: "var(--accent-red)" },
    { name: "Delayed Acknowledgement", percentage: 28, color: "#FF9257", bgColor: "#FF92571A", textColor: "#FF9257" },
    { name: "No Response Timeout", percentage: 20, color: "#FFCA57", bgColor: "#FFCA5733", textColor: "#C68904" },
    { name: "Shift Handoff Gap", percentage: 12, color: "var(--accent-green)", bgColor: "rgba(63, 185, 80, 0.1)", textColor: "var(--accent-green)" },
    { name: "Coverage Override", percentage: 8, color: "var(--accent-primary)", bgColor: "rgba(41, 128, 211, 0.1)", textColor: "var(--accent-primary)" },
];

const TopRejectionReasons: React.FC = () => {
    const [animatedBars, setAnimatedBars] = React.useState(escalationCategories.map(() => 0));
    ...
    React.useEffect(() => {
        ...
        const animate = () => {
            ...
            setAnimatedBars(escalationCategories.map(item => item.percentage * eased));
            setAnimatedTotal(Math.round(totalRejections * eased));
            if (progress < 1) requestAnimationFrame(animate);
            else { setAnimatedBars(escalationCategories.map(item => item.percentage)); setAnimatedTotal(totalRejections); }
        };
        ...
        <DashboardCard className="flex flex-col flex-1" padding="none" style={{ padding: 16, gap: 15 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <Text variant="body-md-semibold" color="text-primary" className="font-bold">Escalation Breakdown</Text>
                    <Text variant="body-sm" color="text-secondary">All Departments · Current Window</Text>
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {escalationCategories.map((category, index) => (
                    <React.Fragment key={category.name}>
                        ...
                        {index < escalationCategories.length - 1 && <div className="border-t border-tertiary" />}
                    </React.Fragment>
                ))}
            </div>
        </DashboardCard>
