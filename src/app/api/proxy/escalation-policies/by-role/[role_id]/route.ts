...
// GET /escalation-policies/by-role/{role_id}
export async function GET(
    ...
    try {
        ...
        const upstream = await buildTenantUpstreamUrl(req, API_BASE_URL, `/api/v1/escalation-policies/by-role/${role_id}`);

        if (upstream instanceof NextResponse) return upstream;

        const { url } = upstream;
        console.log('Proxy get escalation-policy by role request to:', url);
