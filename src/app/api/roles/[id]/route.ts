...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    const { name, description, department, mandatory, priority, enabled, visible_in_directory, escalation_routing, escalation_levels } = await req.json();
    ...
    const rows = await sql`
        UPDATE roles
        SET name = ${name}, description = ${description || ''}, department = ${department || ''}, mandatory = ${mandatory || false},
